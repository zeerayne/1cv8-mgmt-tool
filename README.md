...
|`MAINTENANCE_PG`                             |Включает или отключает функцию обслуживания базы данных средствами СУБД PostgreSQL. Если включено, будет выполнено обслуживание утилитой [vacuumdb](https://www.postgresql.org/docs/current/app-vacuumdb.html)|
...
|Параметр|Описание|
|-------:|:-------|
|`PG_BACKUP_ENABLED`|Включает или отключает функцию создания резервных копий средствами PostgreSQL для совместимых информационных баз (базы данных которых размещены на СУБД PostgreSQL), принимает значения `True` или `False`|
