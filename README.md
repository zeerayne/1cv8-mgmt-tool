...
При окончании резервного копирования и загрузки копий на AWS приложение может отправить нотификаицию с результатами описанными ниже способами.
...
|Параметр|Описание|
|-------:|:-------|
|`NOTIFY_TELEGRAM_ENABLED`|Включает или отключает отправку уведомлений через Telegram, принимает значения `True` или `False`| 
|`NOTIFY_TELEGRAM_BOT_ID`|Токен HTTP API Telegram-бота в формате `123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11`| 
|`NOTIFY_TELEGRAM_CHAT_ID`|ID чата или канала, в который бот будет отправлять уведомления| 
|`NOTIFY_TELEGRAM_CAPTION`|Заголовок уведомления, который будет добавляться в начало текста сообщения| 
